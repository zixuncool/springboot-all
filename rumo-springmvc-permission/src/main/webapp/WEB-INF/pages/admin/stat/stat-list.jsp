...
<div class="page-container">
	<div class="text-c"> 日期范围：
		...
		<button type="button" class="btn btn-success radius" id="search_statbtn"><i class="Hui-iconfont">&#xe665;</i> 搜用户</button>
	</div>
...
<script>
	...
	var rumoStat = {
		...
		status:"",
		init:function(){
			rumoStat.load(function(totalPageSize){
				//初始化分页
				rumoStat.loadPage(totalPageSize*1);
			});
			
			$("#search_statbtn").on("click",function(){
				rumoStat.search();
			});
			
			//有一个前提，没有调用的方法没有参数
			//$("#search_statbtn").on("click",rumoStat.search);
		},
		...
		load:function(callback){
			...
			//借用模板
			var params = {
				"pageNo":rumoStat.pageNo,
				"pageSize":rumoStat.pageSize,
				"keyword":rumoStat.keyword,
				"status":rumoStat.status
			}
			
			$("#tbody").load("http://localhost:8082/admin/stat/template",params,function(){
