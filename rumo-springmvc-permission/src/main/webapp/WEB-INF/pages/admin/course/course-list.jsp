...
<div class="page-container">
	<div class="text-c"> 日期范围：
		...
		<button type="button" class="btn btn-success radius" id="search_coursebtn"><i class="Hui-iconfont">&#xe665;</i> 搜用户</button>
	</div>
...
<script>
	...
	var rumoCourse = {
		...
		status:"",
		init:function(){
			rumoCourse.load(function(totalPageSize){
				//初始化分页
				rumoCourse.loadPage(totalPageSize*1);
			});
			
			$("#search_coursebtn").on("click",function(){
				rumoCourse.search();
			});
			
			//有一个前提，没有调用的方法没有参数
			//$("#search_coursebtn").on("click",rumoCourse.search);
		},
		...
		load:function(callback){
			...
			//借用模板
			var params = {
				"pageNo":rumoCourse.pageNo,
				"pageSize":rumoCourse.pageSize,
				"keyword":rumoCourse.keyword,
				"status":rumoCourse.status
			}
			
			$("#tbody").load("http://localhost:8082/admin/course/template",params,function(){
