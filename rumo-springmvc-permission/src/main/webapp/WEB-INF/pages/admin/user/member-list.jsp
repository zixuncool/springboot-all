...
<div class="page-container">
	<div class="text-c"> 日期范围：
		...
		<button type="button" class="btn btn-success radius" id="search_userbtn"><i class="Hui-iconfont">&#xe665;</i> 搜用户</button>
	</div>
...
<script>
	...
	var rumoMembers = {
		...
		status:"",
		init:function(){
			rumoMembers.load(function(totalPageSize){
				//初始化分页
				rumoMembers.loadPage(totalPageSize*1);
			});
			
			$("#search_userbtn").on("click",function(){
				rumoMembers.search();
			});
			
			//有一个前提，没有调用的方法没有参数
			//$("#search_userbtn").on("click",rumoMembers.search);
		},
		...
		search:function(){
			...
			//改变参数
			rumoMembers.pageNo = 0;
			rumoMembers.keyword = keyword;
		...
		load:function(callback){
			...
			//借用模板
			var params = {
				"pageNo":rumoMembers.pageNo,
				"pageSize":rumoMembers.pageSize,
				"keyword":rumoMembers.keyword,
				"status":rumoMembers.status
			}
			
			$("#tbody").load("http://localhost:8082/admin/user/template",params,function(){
