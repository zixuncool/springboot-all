...
function addScript(srcUrl) {
	document.write('<scr' + 'ipt type="text/javascript" src="' + srcUrl
			+ '"></script>');
}
...
function getStyle(dom, attr) {
	return window.getComputedStyle ? window.getComputedStyle(dom, false)[attr]
...
var tzUtil = {
	...
	animates : function($dom, mark, callback) {
		...
		case "slideUp":
			...
			break;
		case "fadeIn":
			$dom.toggleClass(tz_animateOut()).fadeIn("slow", function() {
				if (callback)
					callback();
			});
			break;
		case "slideDown":
			$dom.toggleClass(tz_animateOut()).slideDown("slow", function() {
				if (callback)
					callback();
			});
			break;
...
function pre(obj) {
	return obj.previousSibling.nodeType == 1 ? obj.previousSibling
...
(function(win) {
	...
	// d:总时长
	/*
	 * Linear：无缓动效果 Quadratic：二次方的缓动（t^2） Cubic：三次方的缓动（t^3） Quartic：四次方的缓动（t^4）
	 * Quintic：五次方的缓动（t^5） Sinusoidal：正弦曲线的缓动（sin(t)） Exponential：指数曲线的缓动（2^t）
	 * Circular：圆形曲线的缓动（sqrt(1-t^2)） Elastic：指数衰减的正弦曲线缓动 超过范围的三次方缓动（(s+1)*t^3 –
	 * s*t^2） 指数衰减的反弹缓动 每个效果都分三个缓动方式，分别是（可采用后面的邪恶记忆法帮助记忆）：
	...
	function animate(dom, json) {
		...
		var fx = typeof args[2] == "string" ? args[2]
				: (typeof args[3] != "string" ? "linear" : args[3]);
		var fn = typeof args[args.length - 1] != "function" ? null
				: args[args.length - 1];
		// 目标初始值
		var target = {};
