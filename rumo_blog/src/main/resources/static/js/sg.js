...
$.tzPrompt = function(options){
	...
	if(opts.textarea){
		var mh = opts.height - 160;
		$dialog.find(".tzdialog_message").html("<div style='width:"+mw+"px;margin:0 auto;text-align:left'>" +
				"<p><textarea class='dtext' maxlength="+opts.maxsize+" style='overflow:auto;line-height:22px;font-size:12px;width:"+mw+"px;height:"+mh+"px;border:1px solid #ccc;' placeholder='"+opts.content+"'>"+opts.value+"</textarea></p></div>");
		
	}else{
		$dialog.find(".tzdialog_message").html("<div style='width:"+mw+"px;margin:0 auto;text-align:left'>" +
				"<p><input type='text'  class='dtext' maxlength="+opts.maxsize+" style='width:"+mw+"px;height:36px;line-height:36px;text-indent:0.5em;border:1px solid #ccc;' value='"+opts.value+"' placeholder='"+opts.content+"'></p></div>");
	}
...
$.tzIframe = function(options){
	var opts = $.extend({},$.tzDialog.methods,$.tzDialog.defaults,options);
	var $dialog = opts.init(opts);
	$dialog.find(".tzdialog_message").html("<div  class='tm-dialog-loading tmWindow_loading'><img src='"+basePath+"/img/loading.gif'><label style='font-size:12px;'>请稍后，正在加载中...</label></div>");
	...
	var h = 76;
	if(!opts.showButton)h=8;
	$dialog.find(".tzdialog_message").height(opts.height-h).css("padding",6).append(iframe);
...
$.tzDialog.methods = {
	...
	init:function(opts){
		...
		if(opts.background)$dialog.css("background",opts.background);
		return $dialog;
	},
	params:function($dialog,opts){
		...
		$dialog.find(".tzdialog_message").css({"textAlign":"center","lineHeight":opts.height-170+"px"}).height(opts.height-170);
		//弹出层居中
	...
	template : function(opts){
		$(".tmui-overlay").remove();
		$(".tzui-dialog").remove();
		var $dialog = $("<div class='tzui-dialog showalert'>"+
		...
		"			<span class='content'><span class='iconfont icon-"+opts.icon+"'></span>"+opts.content+"</span></div>"+
	...
	events:function($dialog,opts){
		...
		$dialog.find(".tzdialog_ok").on("click",function(){
			...
			$dialog.addClass("hidealert").next().remove();
			tzUtil.animates($dialog,opts.animate);
		});
		...
		$dialog.next().on("click",function(){
			$dialog.find(".delete").trigger("click");
		});
...
$.tzDialog.defaults = {
	...
	drag:true,
	ghost:true,
	background:"linear-gradient(rgba(53, 52, 52,0.96), rgba(22, 31, 51,0.92))",
	textarea:false,
	value:"",
	animate:"top",
	showButton:true,
