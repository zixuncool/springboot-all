$(function () {
	...
	$(".menu-item-course").find("a").addClass("active").end().siblings().find("a").removeClass("active");
...
function updatehit(){
	...
	$.get(basePath+"/course/updatehit/"+opid,function(response){
...
var vue = new Vue({
	...
	methods:{
		...
		del:function(e){
			var $this = this;
			var courseId = e.target.getAttribute("opid");
			mkCommon.tip("你确定删除吗?","error",function(ok){
				if(ok){
					vue.$http.post(adminPath+"/course/deleteComment/"+courseId,{
						emulateJSON: true
					}).then(function(response){
						if(response.body.status==200){
							$("#vcard_"+courseId).addClass("animated bounceOutLeft").fadeOut(1000,function(){
								$(this).remove();
							});
						}
					});
		...
		load:function(){
			...
			this.$http.get(basePath+"/course/loadcomment",{params:{"courseId":courseId,"pageNo":pageNo}}).then(function(response){
		...
		saveComment:function(){
			...
			mkLogin.loginSuccess(function(data){
				if(data){
					...
					var params = {"courseId":blodId,"content":message,"parentId":parentId,"replyUserid":ruserid};
					vue.$http.post(adminPath+"/course/saveComment",params,{
						emulateJSON: true
					}).then(function(response){
						if(response.body.status==200){
							mkCommon.tip("评论成功!");
